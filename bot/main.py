...
def main() -> None:
    ...
    application = Updater(os.environ["TOKEN"])
    # on different commands - answer in Telegram
    application.dispatcher.add_handler(CommandHandler("start", start))
    application.dispatcher.add_handler(CommandHandler("subsribe", start))
    application.dispatcher.add_handler(CommandHandler("addTask", help_command))
    application.dispatcher.add_handler(CallbackQueryHandler(button))
    # on non command i.e message - echo the message on Telegram
    application.dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, voiceanalysis))
